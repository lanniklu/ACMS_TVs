...
cleanup_old_logs() {
    log_info "Cleaning up logs older than 72 hours..."
    
    if [[ ! -d "${LOG_DIR}" ]]; then
        log_warn "Log directory doesn't exist: ${LOG_DIR}"
        return 0
    fi
    
    # Find and delete logs older than 72 hours
    local deleted_count=0
    while IFS= read -r logfile; do
        if [[ -n "${logfile}" ]]; then
            rm -f "${logfile}"
            deleted_count=$((deleted_count + 1))
            log_info "Deleted old log: $(basename "${logfile}")"
        fi
    done < <(find "${LOG_DIR}" -type f -name "*.log" -mtime +3 2>/dev/null)
    
    # Also clean JSON schedule files older than 7 days to prevent accumulation
    local schedule_dir="${BASE_DIR}/schedules"
    if [[ -d "${schedule_dir}" ]]; then
        while IFS= read -r schedule_file; do
            if [[ -n "${schedule_file}" ]]; then
                rm -f "${schedule_file}"
                log_info "Deleted old schedule: $(basename "${schedule_file}")"
            fi
        done < <(find "${schedule_dir}" -type f -name "*.json" -mtime +7 2>/dev/null)
    fi
    
    log_info "Log cleanup complete (${deleted_count} files removed)"
    return 0
}
