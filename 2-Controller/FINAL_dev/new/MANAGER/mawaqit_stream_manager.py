...
class MultiDeviceController:
    ...
    def _is_prayer_time(self):
        ...
        Cas spécial: Jumuaa dure 60 min (10 min avant + 50 min après)
        ...
        try:
            ...
            for event in self.ptz_scheduler.current_schedule.get("events", []):
                ...
                # Cas 1: Événements Jumuaa (pre, khotba, position3)
                # Jumuaa dure 60 min: 10 min avant + 50 min après (jusqu'à retour Mawaqit)
                if event["type"] in ["jumua_pre", "jumua_khotba", "jumua_position3"]:
                    ...
                    # Fenêtre Jumuaa: 10 min avant à 50 min après
                    jumuaa_start = jumua_dt - timedelta(minutes=10)
                    jumuaa_end = jumua_dt + timedelta(minutes=50)  # 10 + 50
    ...
    def update_device(self, device: DeviceConfig) -> bool:
        ...
        try:
            ...
            # Priority 4: Outside prayer time, always return to Mawaqit
            else:
                if is_prayer:
