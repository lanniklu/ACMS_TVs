...
# Logging
LOG_FILE = "/home/acms_tech/AUTO_StreamACMS/logs/mawaqit_stream.log"
LOG_LEVEL = logging.INFO

# PID file (prevents multiple instances)
PID_FILE = "/home/acms_tech/AUTO_StreamACMS/logs/mawaqit_stream_manager.pid"
...
HEARTBEAT_FILE = "/home/acms_tech/AUTO_StreamACMS/logs/mawaqit_heartbeat.txt"  # Updated each cycle
...
class StreamPriority(Enum):
    ...
    PC_VNC = 1       # Maximum priority (best latency)
    PC_HTTP = 2      # High priority
    ONVIF = 3        # Medium priority
    MAWAQIT = 4      # Minimum priority (fallback)
...
class MultiDeviceController:
    ...
    def __init__(self):
        ...
        self.ptz_update_interval = 86400  # Mettre à jour schedule chaque 24h
        self._initialize_ptz()

    def _initialize_ptz(self):
        """Initialise le système PTZ pour la caméra IMAM"""
        if not PTZ_AVAILABLE:
            logging.warning("⚠ Module PTZ non disponible - caméra IMAM non gérée")
            return
        
        try:
            logging.info("[PTZ] Initialisation du système PTZ...")
            self.ptz_controller = PTZController(PTZ_CONFIG)
            self.ptz_parser = MawaqitParser()
    ...
    def _check_and_update_ptz(self):
        ...
        try:
            ...
            # Mise à jour du planning chaque 24h
            if now - self.last_ptz_schedule_update > self.ptz_update_interval:
                logging.info("[PTZ] Mise à jour quotidienne du planning...")
    ...
    def check_sources(self):
        """Check state of all sources - with error isolation"""
        prev_vnc = self.vnc_available
        prev_http = self.http_available
        prev_onvif = self.onvif_available

        try:
            self.vnc_available = SourceChecker.check_vnc()
        except Exception as e:
            logging.error(f"Error checking VNC: {e}")
            self.vnc_available = False
        ...
        # Unblock streams when source becomes available again
        ...
        if not prev_vnc and self.vnc_available:
            logging.info("VNC stream became available - unblocking")
            for device in self.devices:
    ...
    def _is_prayer_time(self):
        """
        Détecte si on est actuellement DANS une période de prière
        Cas spécial: Jumuaa dure 25 min (5 min avant + 20 min après)
        Retourne: (is_prayer: bool, prayer_type: str, description: str)
        """
        ...
        try:
            ...
            for event in self.ptz_scheduler.current_schedule.get("events", []):
                ...
                # Cas 1: Événements Jumuaa (khotba ou position3)
                # Jumuaa dure 25 min: 5 min avant + 20 min après (jusqu'à retour Mawaqit)
                if event["type"] in ["jumua_khotba", "jumua_position3"]:
                    jumua_time_str = event.get("jumua_time")
                    if not jumua_time_str:
                        continue
                    
                    jumua_dt = datetime.strptime(f"{now.strftime('%Y-%m-%d')} {jumua_time_str}", "%Y-%m-%d %H:%M")
                    # Fenêtre Jumuaa: 5 min avant à 25 min après
                    jumuaa_start = jumua_dt - timedelta(minutes=5)
                    jumuaa_end = jumua_dt + timedelta(minutes=25)  # 5 + 20
                    
                    if jumuaa_start <= now <= jumuaa_end:
                        prayer_type = event.get("prayer", "unknown")
                        description = event.get("description", "")
                        logging.debug(f"[PRAYER] Jumuaa detected: {description} (ends at {jumuaa_end.strftime('%H:%M')})")
                        return True, prayer_type, description
                
                # Cas 2: Événements Salat réguliers (fajr, dhuhr, asr, maghrib, isha)
                elif event["type"] == "iqama":
                    start_time_str = event.get("iqama_time")
                    if not start_time_str:
                        continue
                    
                    # Parse les horaires
                    event_start = datetime.strptime(f"{now.strftime('%Y-%m-%d')} {start_time_str}", "%Y-%m-%d %H:%M")
                    event_end = event_start + timedelta(minutes=PRAYER_DURATION)
                    
                    # Vérifie si on est DANS la fenêtre de prière
                    if event_start <= now <= event_end:
                        prayer_type = event.get("prayer", "unknown")
                        description = event.get("description", "")
                        logging.debug(f"[PRAYER] Detected: {description} (ends at {event_end.strftime('%H:%M')})")
                        return True, prayer_type, description
            
            return False, None, None
        
        except Exception as e:
            logging.error(f"Error detecting prayer time: {e}")
            return False, None, None

    def update_device(self, device: DeviceConfig) -> bool:
        """Update device display based on available sources with prayer-time awareness"""
        try:
            is_prayer, prayer_type, prayer_desc = self._is_prayer_time()
            
            # Priority 1: VNC (best latency) - ALWAYS has absolute priority
            if self.vnc_available:
                return self.stream_manager.play_vnc(device)

            # Priority 2: HTTP - ALWAYS has priority
            elif self.http_available:
                return self.stream_manager.play_http_vlc(device)

            # Priority 3: During prayer time - use ONVIF (camera) if available
            elif is_prayer and self.onvif_available:
                logging.info(f"📹 Prayer detected: {prayer_desc} → Switching to ONVIF (camera)")
                return self.stream_manager.play_onvif(device)

            # Priority 4: Mawaqit (fallback - used when not in prayer or ONVIF unavailable)
            else:
                if is_prayer:
                    logging.warning(f"⚠️ Prayer in progress ({prayer_desc}) but ONVIF unavailable → Using Mawaqit")
            ...
            try:
                ...
                if expected_package:
                    ...
                    if foreground_app and expected_package in foreground_app:
                        ...
                        # For MAWAQIT, just relaunch
                        if expected_package == "com.mawaqit":
                            logging.info(f"Relaunching MAWAQIT on {device.name}")
    ...
    def print_status(self):
        ...
        logging.info("-" * 70)
        vnc_status = "+ OK" if self.vnc_available else "- KO"
        http_status = "+ OK" if self.http_available else "- KO"
        onvif_status = "+ OK" if self.onvif_available else "- KO"
        logging.info(f"SOURCES: VNC={vnc_status} | HTTP={http_status} | ONVIF={onvif_status}")
